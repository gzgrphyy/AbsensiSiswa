  ...
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="onBackdropClick">
        ...
        <div v-if="isAdmin" :class="['relative bg-white dark:bg-slate-800 rounded-none border border-gray-300 dark:border-gray-600 w-full mx-auto', maxWidth]">
          <div v-if="title || $slots.header" class="flex items-center justify-between px-4 pt-4 pb-2 border-b border-gray-300 dark:border-gray-600">
            ...
            <button @click="emit('close')" class="p-1 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-none transition-colors">
