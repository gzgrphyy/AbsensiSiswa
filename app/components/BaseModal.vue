  ...
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="onBackdropClick">
        <div class="absolute inset-0 bg-black/20 backdrop-blur-sm" @click="onBackdropClick" />
        <div :class="['relative bg-white rounded-2xl shadow-xl border border-gray-100 w-full mx-auto', maxWidth]">
          <!-- Header -->
          ...
          <div v-if="$slots.footer" class="flex justify-end gap-3 px-6 pb-6 pt-4 border-t border-gray-50">
            <slot name="footer" />
          </div>
        </div>
      </div>
