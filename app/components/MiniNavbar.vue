...
<template>
  <header class="bg-gray-200 text-gray-700 flex-shrink-0 relative z-50">
    <div class="px-3 sm:px-4 flex items-center justify-between h-[26px]">
      ...
      <NuxtLink to="/admin" class="flex items-center gap-1.5 min-w-0">
        ...
        <span class="text-[10px] font-medium text-gray-600 truncate hidden sm:inline">{{ pengaturan?.namaAplikasi || 'Aplikasi Skoria' }}</span>
      </NuxtLink>
    ...
    <div v-if="showProfile" class="fixed top-8 right-3 sm:right-4 w-72 bg-white rounded border border-gray-200 z-[100]" @click.stop>
      <div class="p-4">
        <div class="flex items-center gap-3 mb-3">
          ...
          <div>
            <p class="text-sm font-semibold text-gray-900">{{ dev.nama }}</p>
            <p class="text-xs text-gray-500">{{ dev.role }}</p>
          </div>
        </div>

        <div class="border-t border-gray-100 pt-3 space-y-2 text-sm">
          <a :href="'mailto:' + dev.email" class="flex items-center gap-2 text-gray-600 hover:text-primary-600">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
            {{ dev.email }}
          </a>
          <a :href="'tel:' + dev.telepon" class="flex items-center gap-2 text-gray-600 hover:text-primary-600">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
            </svg>
            {{ dev.telepon }}
          </a>
          <a :href="'https://' + dev.github" target="_blank" class="flex items-center gap-2 text-gray-600 hover:text-primary-600">
          ...
          <a :href="'https://' + dev.website" target="_blank" class="flex items-center gap-2 text-gray-600 hover:text-primary-600">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
