  ...
  <div>
    ...
    <div class="flex items-center gap-3">
      ...
      <div v-if="previewUrl" class="flex items-center gap-2">
        <img :src="previewUrl" alt="Preview" :class="['w-8 h-8 object-contain border border-gray-200 dark:border-slate-600', isAdmin ? 'rounded-sm' : 'rounded']" />
        <span class="text-sm text-gray-500 dark:text-gray-400 truncate max-w-[150px]">{{ currentPath?.split('/').pop() }}</span>
        <button
          type="button"
          :disabled="deleting"
          :class="['p-1.5 text-red-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors disabled:opacity-50', isAdmin ? 'rounded-sm' : 'rounded-lg']"
          title="Hapus file"
          @click="handleDelete"
        >
