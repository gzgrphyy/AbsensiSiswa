...
const result = ref<{
  ...
  jadwals?: { id: number; mapel: string; jamMulai: string; jamSelesai: string; kelas: string; ruangan: string }[]
} | null>(null)
const errorMsg = ref('')
...
<template>
  <PTKLayout>
    <PageHeader
      ...
      description="Arahkan kamera ke QR Code ruangan untuk mencatat kehadiran PTK"
      back-to="/absensi"
    />
        ...
        <template v-if="result">
          <div class="text-center py-4">
            ...
            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">
              {{ result.success ? 'Kehadiran Tercatat!' : 'Scan Gagal' }}
            </h2>
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-5">{{ result.message }}</p>

            <div v-if="result.ruangan" class="bg-gray-50 dark:bg-slate-700/50 rounded-lg border border-gray-200 dark:border-slate-700 p-4 mb-5 text-left space-y-2.5 text-sm">
            ...
            <div v-if="result.jadwals && result.jadwals.length > 0" class="bg-gray-50 dark:bg-slate-700/50 rounded-lg border border-gray-200 dark:border-slate-700 p-4 mb-5 text-left">
              <p class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2">Jadwal yang Anda ampu di ruangan ini</p>
              <ul class="space-y-2.5">
                <li v-for="j in result.jadwals" :key="j.id" class="flex items-center justify-between text-sm">
                  <div class="min-w-0">
                    <p class="font-medium text-gray-900 dark:text-gray-100 truncate">{{ j.mapel }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ j.kelas }} · {{ j.jamMulai }} - {{ j.jamSelesai }}</p>
                  </div>
                </li>
              </ul>
            </div>
