  ...
  <AppLayout>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div v-for="opt in options" :key="opt.id"
        ...
        class="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 shadow-card dark:shadow-dark-card p-5 hover:shadow-card-hover dark:hover:shadow-dark-card hover:border-blue-200 dark:hover:border-blue-700 transition-all cursor-pointer max-w-sm"
        ...
        <div class="flex items-start gap-4">
          ...
          <svg v-else class="w-5 h-5 text-gray-300 dark:text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
        </div>
      </div>
    </div>
  </AppLayout>
