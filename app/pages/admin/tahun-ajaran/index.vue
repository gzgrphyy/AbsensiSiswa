      ...
      <div v-if="pending" class="bg-white dark:bg-gray-800 rounded-none border border-gray-300 dark:border-gray-600 overflow-hidden">
        <div class="p-6 space-y-4">
          <div v-for="i in 3" :key="i" class="flex items-center gap-4 animate-pulse">
            ...
            <div class="h-6 bg-gray-200 dark:bg-slate-700 rounded-none w-20"></div>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
              <tr v-for="item in data" :key="item.id"
                ...
                <td class="px-4 sm:px-6 py-4 hidden sm:table-cell">
                  <span class="inline-flex items-center px-2.5 py-0.5 rounded-none text-xs font-medium"
                ...
                <td class="px-4 sm:px-6 py-4 text-center">
                  <div class="flex items-center justify-center">
                    <span v-if="item.isActive"
                      class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-none text-xs font-semibold bg-green-100 text-green-700 ring-1 ring-green-200 shadow-sm">
                      <span class="w-1.5 h-1.5 rounded-none bg-green-500 animate-pulse"></span>
                      Aktif
                    </span>
                    <span v-else
                      class="inline-flex items-center px-2.5 py-0.5 rounded-none text-xs font-medium bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-400 ring-1 ring-gray-200 dark:ring-slate-600">
                      Tidak Aktif
                    </span>
                  </div>
                </td>
                <td class="px-4 sm:px-6 py-4">
                  <div class="flex items-center justify-center gap-1">
                    ...
                    <button v-if="!item.isActive" @click="promptDelete(item)"
                      class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-none transition-all duration-150"
    ...
    <Teleport to="body">
          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-none w-full max-w-md mx-auto overflow-hidden border border-gray-300 dark:border-gray-600">
            ...
            <div class="flex items-center justify-between px-4 pt-4 pb-2">
              ...
              <button @click="handleCloseClick"
                class="p-1.5 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-none transition-colors">
            ...
            <form @submit.prevent="handleSave" class="p-4 space-y-4">
              ...
              <!-- Error -->
              <Transition name="fade">
                <div v-if="errorMsg" class="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-none text-sm text-red-700">
                  <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="confirmToggle" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-none w-full max-w-sm mx-auto p-4 border border-gray-300 dark:border-gray-600">
            <div class="flex items-center gap-3 mb-4">
              <div class="p-2 bg-amber-100 dark:bg-amber-900/30 rounded-none">
            ...
            <p class="text-sm text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/30 border border-amber-100 dark:border-amber-800 rounded-none p-3 mb-4">
              Tahun ajaran lain yang aktif akan otomatis dinonaktifkan.
            </p>
            <div class="flex justify-end gap-3">
              ...
              <button @click="handleToggle"
                class="px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-none hover:bg-green-700 active:bg-green-800">
                Ya, Aktifkan
              </button>
            </div>
          </div>
      ...
      <Transition name="modal">
        <div v-if="confirmDelete" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-none w-full max-w-sm mx-auto p-4 border border-gray-300 dark:border-gray-600">
            <div class="flex items-center gap-3 mb-4">
              <div class="p-2 bg-red-100 dark:bg-red-900/30 rounded-none">
            ...
            <div v-if="confirmDelete.kelasCount > 0"
              class="mt-3 p-3 bg-amber-50 dark:bg-amber-900/30 border border-amber-100 dark:border-amber-800 rounded-none text-sm text-amber-700 dark:text-amber-300 flex items-start gap-2">
              <svg class="w-4 h-4 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" />
              </svg>
              <span>
                Tahun ajaran ini masih memiliki <strong>{{ confirmDelete.kelasCount }} kelas</strong> terkait. Data murid/absensi tidak akan hilang, tetapi tahun ajaran tidak akan muncul di pilihan baru.
              </span>
            </div>
            ...
            <div class="flex justify-end gap-3 mt-5 pt-4 border-t border-gray-100 dark:border-slate-700">
              ...
              <button @click="handleDelete"
                class="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-none hover:bg-red-700 active:bg-red-800">
                Ya, Hapus
              </button>
            </div>
          </div>
